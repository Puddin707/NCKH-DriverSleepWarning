...
py mouth_demo.py
--------------
Demo theo dõi miệng bằng MediaPipe Face Mesh + MAR:
- Vẽ 8 điểm viền miệng (mouth_utils)
- Tính MAR (Mouth Aspect Ratio) + phát hiện "há miệng" đơn giản bằng ngưỡng & số khung liên tiếp

Phím tắt:
  q / ESC  - Thoát
  f        - Bật/tắt lật gương (mirror)
  v        - Bật/tắt vẽ polyline miệng
...
Đặt file này cùng thư mục với:
  geom_types.py, mouth_utils.py, mar_calc.py, moving_average.py
"""
...
from mouth_utils import (
    MOUTH_IDX,
    draw_mouth_polyline,
    pts_from_landmarks as mouth_pts_from,
)
...
def main():
    ...
    ap.add_argument("--mar-th", type=float, default=0.65, help="Ngưỡng MAR coi là há miệng (mặc định 0.65)")
    ap.add_argument("--open-frames", type=int, default=5, help="Số khung MAR>th để tính 1 lần há (mặc định 5)")
    ap.add_argument("--smooth-win", type=int, default=3, help="Cửa sổ MovingAverage cho MAR (mặc định 3)")
    ap.add_argument("--win", default="Mouth Demo", help="Tên cửa sổ")
    args = ap.parse_args()
    ...
    filt_mar = MovingAverage(max(1, args.smooth_win))
    ...
    # Biến đếm há miệng (yawn/open mouth events)
    open_count = 0
    over_thresh_frames = 0
    opening_flag = False  # hiển thị trạng thái tạm thời
    ...
    try:
        while True:
            ...
            if face_found:
                ...
                # 8 điểm miệng
                mouth_pts = mouth_pts_from(fl, MOUTH_IDX, w, h)
                ...
                # Phát hiện há miệng đơn giản
                if mar_v is not None:
                    if mar_v > args.mar_th:
                        over_thresh_frames += 1
                        opening_flag = True
                    else:
            ...
            if show_overlay:
                lines = [
                    ...
                    f"MAR: {mar_v:.3f}" if mar_v is not None else "MAR: --",
                    f"Open Th: {args.mar_th:.2f} | MinFrames: {args.open_frames} | SmoothWin: {max(1, args.smooth_win)}",
                    f"Open Count: {open_count} {'(opening...)' if opening_flag else ''}",
